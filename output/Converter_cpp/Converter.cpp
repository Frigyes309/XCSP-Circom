...
#include "circom.hpp"
#include "calcwit.hpp"
void Store_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Store_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
void Sum_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Sum_3_run(uint ctx_index,Circom_CalcWit* ctx);
void Minimum_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Minimum_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 271;}
...
ctx->componentMemory[coffset].templateName = "Store";
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 31
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 0; i++){
...
ctx->componentMemory[coffset].templateName = "Minimum";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 2; i++) {
std::string new_cmp_name = "local_min"+ctx->generate_position_array(aux_dimensions, 1, i);
Store_0_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 2 ;
aux_cmp_num += 1;
}
...
std::string new_cmp_name = "comparator"+ctx->generate_position_array(aux_dimensions, 1, i);
...
for (uint i_aux = 0; i_aux < 1; i_aux++) {
uint i = aux_positions[i_aux];
std::string new_cmp_name = "sum"+ctx->generate_position_array(aux_dimensions, 1, i);
Sum_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 3 ;
aux_cmp_num += 1;
}
...
Store_0_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Sum_3_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 2];
// load src
Fr_sub(&expaux[2],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0]); // line circom 22
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&expaux[2]); // line circom 22
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// run sub component if needed
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "comparator_min_1";
Minimum_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 267 ;
aux_cmp_num += 6;
}
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
Minimum_4_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
