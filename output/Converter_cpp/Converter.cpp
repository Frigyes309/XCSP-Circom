...
void AllDifferent_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void AllDifferent_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 13487;}

uint get_number_of_components() {return 4460;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3028;}

uint get_size_of_constants() {return 57;}
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 35
// end load src
...
ctx->componentMemory[coffset].templateName = "AllDifferent";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 55;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[2972]{0};
}
...
uint aux_dimensions[1] = {1485};
for (uint i = 0; i < 1485; i++) {
...
{
uint aux_create = 1485;
int aux_cmp_num = 2972+ctx_index+1;
uint csoffset = mySignalStart+8972;
uint aux_dimensions[1] = {1486};
for (uint i = 0; i < 1486; i++) {
...
{
uint aux_create = 2971;
int aux_cmp_num = 2970+ctx_index+1;
uint csoffset = mySignalStart+8966;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "helper";
IsEqual_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 6 ;
aux_cmp_num += 2;
...
{
uint cmp_index_ref = ((1 * (Fr_toInt(&lvar[1]) + 1)) + 1485);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[1])) + 1485)]].signalStart + 0]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Sum_0_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[2970]].signalStart + 0]);
}
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[2971]].signalStart + 0]);
}
for (uint i = 0; i < 2972; i++){
uint index_subc = ctx->componentMemory[ctx_index].subcomponents[i];
if (index_subc != 0)release_memory_component(ctx,index_subc);
}
}
...
uint csoffset = mySignalStart+56;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "comparator_all_different_1";
AllDifferent_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 13430 ;
aux_cmp_num += 4459;
}
...
ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1;
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
AllDifferent_3_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
